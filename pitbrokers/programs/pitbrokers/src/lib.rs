//! PitBrokers — mint / sell-back program.
//!
...
pub mod pitbrokers {
    ...
    /// Mint a fresh PitBroker. The buyer pays the asset's own rent, which is
    /// why we never pre-mint 1,000 assets (PLAN.md §1.2).
    pub fn mint_new(ctx: Context<MintNew>) -> Result<()> {
        let cfg = &ctx.accounts.config;
        require!(!cfg.paused_mint, PitBrokersError::MintPaused);
        require!(
            Phase::from_u8(cfg.phase)? == Phase::Public,
            PitBrokersError::PhaseClosed
        );
        require!(
            cfg.minted_count < cfg.supply_cap,
            PitBrokersError::SoldOut
        );
    ...
    /// Re-sell a PitBroker that a previous holder sold back. Cheaper for the
    ...
    pub fn mint_recycled(ctx: Context<MintRecycled>) -> Result<()> {
        ...
        require_keys_eq!(
            ...
            PitBrokersError::AssetNotInVault
        );
    ...
    /// Sell a PitBroker back for `redeem_price`. The floor mechanic.
    pub fn redeem(ctx: Context<Redeem>) -> Result<()> {
        let cfg = &ctx.accounts.config;
        require!(!cfg.paused_redeem, PitBrokersError::RedeemPaused);

        let asset = read_asset(&ctx.accounts.asset)?;
        require_keys_eq!(
            asset.owner,
            ctx.accounts.holder.key(),
            PitBrokersError::NotAssetOwner
        );
        require_collection(&asset.update_authority, &cfg.collection)?;
        ...
        require!(
            ...
            PitBrokersError::InsufficientTreasury
        );
    ...
    pub fn withdraw_surplus(ctx: Context<WithdrawSurplus>, amount: u64) -> Result<()> {
        ...
        require!(amount <= surplus, PitBrokersError::WouldBreakFloor);
...
fn read_asset(asset: &UncheckedAccount) -> Result<mpl_core::accounts::BaseAssetV1> {
    require_keys_eq!(
        *asset.owner,
        mpl_core::ID,
        PitBrokersError::InvalidAssetAccount
    );
    let data = asset.try_borrow_data()?;
    mpl_core::accounts::BaseAssetV1::from_bytes(&data)
        .map_err(|_| error!(PitBrokersError::InvalidAssetAccount))
}

/// An asset counts as ours only if its update authority IS our collection.
/// Anyone can create a Core asset named "PitBroker #7"; only assets whose
...
fn require_collection(update_authority: &UpdateAuthority, collection: &Pubkey) -> Result<()> {
    match update_authority {
        ...
        _ => err!(PitBrokersError::WrongCollection),
    }
}
...
pub struct Initialize<'info> {
    ...
    #[account(
        constraint = treasury.mint == token_mint.key() @ PitBrokersError::TreasuryMismatch,
        constraint = treasury.owner == vault.key() @ PitBrokersError::TreasuryMismatch,
    )]
    pub treasury: InterfaceAccount<'info, TokenAccount>,

    /// CHECK: verified as an mpl-core account by the collection constraint below.
    #[account(owner = mpl_core::ID @ PitBrokersError::WrongCollection)]
...
pub struct MintNew<'info> {
    ...
    #[account(mut, address = config.collection @ PitBrokersError::WrongCollection)]
    pub collection: UncheckedAccount<'info>,
    ...
    #[account(mut, address = config.treasury @ PitBrokersError::TreasuryMismatch)]
    pub treasury: InterfaceAccount<'info, TokenAccount>,
