...
use crate::errors::PumpBrokersError;
...
impl Phase {
    pub fn from_u8(v: u8) -> Result<Self> {
        match v {
            ...
            2 => Ok(Phase::Public),
            _ => err!(PumpBrokersError::InvalidPhase),
        }
    }
}
...
impl Config {
    ...
    pub fn base_uri_str(&self) -> Result<&str> {
        ...
        require!(len <= BASE_URI_BYTES, PumpBrokersError::InvalidBaseUri);
        core::str::from_utf8(&self.base_uri[..len])
            .map_err(|_| error!(PumpBrokersError::InvalidBaseUri))
    }
    ...
    pub fn asset_name(&self, index: u16) -> String {
        format!("PumpBroker #{}", index)
    }

    pub fn is_minted(&self, index: u16) -> Result<bool> {
        require!(index < self.supply_cap, PumpBrokersError::IndexOutOfRange);
    ...
    pub fn mark_minted(&mut self, index: u16) -> Result<()> {
        require!(index < self.supply_cap, PumpBrokersError::IndexOutOfRange);
        require!(!self.is_minted(index)?, PumpBrokersError::IndexAlreadyMinted);
        let byte = (index / 8) as usize;
        let bit = (index % 8) as u8;
    ...
    pub fn pick_unminted(&self, start: u16) -> Result<u16> {
        ...
        require!(cap > 0, PumpBrokersError::InvalidSupplyCap);
        ...
        err!(PumpBrokersError::PoolExhausted)
    }
}
...
mod tests {
    ...
    fn uri_is_derived_from_the_committed_base() {
        ...
        assert_eq!(c.asset_name(7), "PumpBroker #7");
    }
